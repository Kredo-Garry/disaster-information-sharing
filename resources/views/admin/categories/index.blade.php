...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-center gap-4">
            ...
            <div class="flex-1">
                ...
                <div class="flex items-center gap-2">
                    <span class="w-3 h-3 rounded-full" style="background-color: {{ $category->color_code ?? '#ccc' }}"></span>
                    <span class="text-xs text-gray-400">{{ $category->color_code ?? 'No Color' }}</span>
                </div>
            </div>
            ...
            <div class="flex items-center gap-2">
                ...
                <form action="{{ route('admin.categories.destroy', $category) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this category?');">
                    @csrf 
                    @method('DELETE')
                    <button class="text-gray-300 hover:text-red-500 transition" title="Delete">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="col-span-3 py-20 text-center bg-white rounded-2xl border border-dashed border-gray-300">
            <p class="text-gray-400">No categories found. Click "Add Category" to create your first one.</p>
        </div>
        @endforelse
    </div>
</div>
